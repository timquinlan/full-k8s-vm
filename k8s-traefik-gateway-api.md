# Kubernetes Single-Node Setup (Traefik + Gateway API)

A guide to installing a single-node Kubernetes cluster on a Lima VM, with k9s, Helm, MetalLB, and Traefik as a Gateway API controller for local testing.
...
## 6. Gateway API with MetalLB and Traefik (VM-local testing)
...
This separation makes it easier to delegate route management to application teams while keeping infrastructure concerns with the platform team.
...
By default the Traefik helm chart has the Gateway API provider disabled. Enable it at install time. Traefik will install its own Gateway API CRDs automatically:
...
Verify the Traefik pod is running and its service has received an IP:
...
You should see `traefik` with `ACCEPTED` showing `True`.

### Deploy a test application

Deploy a vanilla nginx instance and wire it up through a `Gateway` and `HTTPRoute` to verify the full stack is working:
